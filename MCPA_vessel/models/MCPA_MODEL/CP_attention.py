...
class cross_perceptron1(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth, patch_size, heads, dim_head=128, dropout=0.1,
                 emb_dropout=0.1):
        # in_chan=128 out_chan=128 img=28 patch=1
        super().__init__()
        # image_height, image_width = pair(imagesize)  # 28 28
        self.patch_height, self.patch_width = pair(patch_size)  # 1 1

        # num_patches = (image_height // patch_height) * (image_width // patch_width)  # 28 28
        self.patch_dim = in_channels * self.patch_height * self.patch_width  # 64
        self.dmodel = out_channels  # 128
        self.mlp_dim = out_channels * 8  # 512
        ...
        self.transformer = CPA1(out_channels, depth, heads, dim_head, self.mlp_dim, dropout)
    ...
    def forward(self, c1, c2):
        ...
        b, c, h, w = c2.shape
        # B, C, H, W = c1.shape
        c1 = rearrange(c1, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=2, p2=2)
        c1 = self.linear1(c1)
        c2 = rearrange(c2, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=1, p2=1)
        c2 = self.linear2(c2)
        ...
        # out = rearrange(ax, 'b (h w) c -> b c h w', h=c1.shape[2] // 1)
        # print('out')
        # out = self.recover_patch_embedding(ax)
        out = rearrange(ax, 'b (h w) c -> b c h w', h=h // self.patch_height)
        return out


class cross_perceptron2(nn.Module):

    def __init__(self, in_channels, out_channels, depth=1, patch_size=1, heads=2, dim_head=128, dropout=0,
                 emb_dropout=0):
        super().__init__()
        ...
        self.patch_dim = in_channels * self.patch_height * self.patch_width  # 256
        self.dmodel = out_channels  # 256
        self.mlp_dim = out_channels * 4  # 1024
        ...
        self.transformer = CPA2(out_channels, depth, heads, dim_head, self.mlp_dim, dropout, )  # dim=256
    ...
    def forward(self, c1, c2, c3):  # 16,8,4
        ...
        c2 = rearrange(c2, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=1, p2=1)
        c2 = self.linear2(c2)
        c3 = rearrange(c3, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=self.patch_height, p2=self.patch_width)
        c3 = self.linear3(c3)
...
class cross_perceptron3(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth=1, patch_size=1, heads=2, dim_head=128, dropout=0.1,
                 emb_dropout=0.1):
        super().__init__()
        ...
        self.patch_dim = in_channels * self.patch_height * self.patch_width  # 512
        self.dmodel = out_channels  # 512
        self.mlp_dim = self.dmodel * 4  # ?

        self.linear1 = nn.Linear(in_channels * 2, out_channels)
        self.linear2 = nn.Linear(in_channels, out_channels)
        self.linear3 = nn.Linear(in_channels // 2, out_channels)
        self.linear4 = nn.Linear(in_channels, out_channels)
        ...
        self.transformer = CPA3(out_channels, depth, heads, dim_head, self.mlp_dim, dropout, )  # dim=256
    ...
    def forward(self, c1, c2, c3, c4):  # 16,8,4
        ...
        c1 = rearrange(c1,'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=4, p2=4)
        c1 = self.linear1(c1)
        c2 = rearrange(c2, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=2, p2=2)
        c2 = self.linear2(c2)
        c3 = rearrange(c3, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=1, p2=1)
        c3 = self.linear3(c3)
        c4 = rearrange(c4, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=self.patch_height, p2=self.patch_width)
        c4 = self.linear4(c4)
...
class Transformer_global_aware(nn.Module):
    def __init__(self, dim, heads=2, dropout=0.):
        ...
        self.feedforward = FeedForward(dim, hidden_dim=512, dropout=dropout)
    ...
    def forward(self, inputs):
        if type(inputs) == list:
            ...
            r1 = int(h1 * w1 * 4 / 16)
            r2 = int(h1 * w1 * 8 / 64)
            # x3_ = out[:, :r1, :].reshape(B, h1 / 4, w1 / 4, C * 4).permute(0, 3, 1, 2)
            # x4_ = out[:, r1:r1 + r2, :].reshape(B, h1 / 8, w1 / 8,C * 8).permute(0, 3, 1, 2)
