...
class MultiGuide(nn.Module):
    def __init__(self, dims, head):
        super(MultiGuide, self).__init__()
        self.scale = 2
        factor = 2
...
class GLUNet(nn.Module):
    def __init__(self, config, num_classes=1000, Class=2, embed_dims=[64, 128, 256, 512],
        ...
        # self.output = nn.Conv2d(2, 2, kernel_size=1)
        ...
        self.Guide = MultiGuide(64, 1)

        self.bridge = BridegeBlock_4(dims=64, head=1)
    ...
    def forward(self, x):
        ...
        for blk in self.up_block2:
            x = blk(x, h_ori // 16, w_ori // 16)
        # x = x.flatten(2).transpose(1, 2)
        x = self.up_norm2(x)
        x = x.reshape(B, h_ori // 16, w_ori // 16, -1).permute(0, 3, 1, 2).contiguous()
        ...
        for blk in self.up_block3:
            x = blk(x, h_ori // 8, w_ori // 8)
        # x = x.flatten(2).transpose(1, 2)
        x = self.up_norm3(x)
        x = x.reshape(B, h_ori // 8, w_ori // 8, -1).permute(0, 3, 1, 2).contiguous()
        ...
        for blk in self.up_block4:
            x = blk(x, h_ori // 4, w_ori // 4)
        # x = x.flatten(2).transpose(1, 2)
        x = self.up_norm4(x)
        x = x.reshape(B, h_ori // 4, w_ori // 4, -1).permute(0, 3, 1, 2).contiguous()

        x, H, W = self.patch_expand4(x.permute(0, 2, 3, 1).view(B, -1, 64))

        x = x.transpose(1, 2).reshape(B, self.Class, h_ori, w_ori).contiguous()
