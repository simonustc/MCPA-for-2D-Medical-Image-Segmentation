...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        self.conv3 = nn.ConvTranspose2d(64, 4, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), output_padding=(1, 1))

        self.output_dim = dim
        self.norm = norm_layer(4)
        self.apply(self._init_weights)
...
class MCPANET(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2).reshape(B, self.num_classes, 224, 224).contiguous()
        # x = F.softmax(x,dim=1)
        # x = self.head(x)
